...
Let us now take a look at how to solve this problem and reverse a singly linked list. The most common approach to this is to have two pointers, a ***prev*** variable which will point to the node that **preceeds** the node we are currently looking at and a ***next*** variable which will point to the node that **proceeds** the node we are currently looking at.

Now Let's look at some code in Java.
~~~
public static SinglyList reverse(SinglyList head) {
    if (head != null) {
